...
function getManifest(){
    return {name:"Net22 Sources",id:"com.rheno911.net22",version:5,baseUrl:mainUrl,type:"Movie",language:"en"};
}
...
function parseHtml(html){
    var out=[];
    var sp=["flw-item","item-film","movie-item","film-item","post-item","<article"];
    var bl=[];
    for(var s=0;s<sp.length;s++){if(html.indexOf(sp[s])>-1){bl=html.split(sp[s]);break;}}
...
function load(url,callback){
    http_get(url,ch,function(s,html){
        ...
        callback(JSON.stringify({
            url:url,data:idm?idm[1]:(ifm?ifm[1]:url),
            title:tm?tm[1].trim():"",description:dm?dm[1].trim():"",
            posterUrl:im?im[1]:"",year:ym?parseInt(ym[1]):0
        }));
    });
}
